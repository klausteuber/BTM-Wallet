...
class DeeplinkSchemaMatch {
  static hasSchema(schemaString: string): boolean {
    ...
    return (
      ...
      lowercaseString.startsWith('abtm:') ||
  ...
  static navigationRouteFor(
    ...
    if (event.url.toLowerCase().startsWith('abtm:bitcoin:') || event.url.toLowerCase().startsWith('abtm:lightning:')) {
      event.url = event.url.substring(11);
    } else if (event.url.toLocaleLowerCase().startsWith('abtm://widget?action=')) {
      event.url = event.url.substring('abtm://'.length);
    }
  ...
  /**
   * Extracts server from a deeplink like `abtm:setelectrumserver?server=electrum.example.com%3A443%3As`
  ...
  static getServerFromSetElectrumServerAction(url: string): string | false {
    if (
      !url.startsWith('abtm:setelectrumserver') &&
  ...
  /**
   * Extracts url from a deeplink like `abtm:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com`
   * returns FALSE if none found
   *
   * @param url {string}
   * @return {string|boolean}
  ...
  static getUrlFromSetLndhubUrlAction(url: string): string | false {
    if (!url.startsWith('abtm:setlndhuburl') && !url.startsWith('bluewallet:setlndhuburl') && !url.startsWith('setlndhuburl')) return false;
