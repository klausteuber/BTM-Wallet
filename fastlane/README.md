...
### upload_to_browserstack_and_comment

```sh
[bundle exec] fastlane upload_to_browserstack_and_comment
```
...
Deploy iOS build to TestFlight (default) or App Store review
...
Update release notes for App Store versions (iOS or Mac Catalyst)
...
### ios submit_to_app_store_lane

```sh
[bundle exec] fastlane ios submit_to_app_store_lane
```

Upload IPA, metadata, and submit to App Review

### ios upload_bugsnag_sourcemaps

```sh
[bundle exec] fastlane ios upload_bugsnag_sourcemaps
```

Upload iOS source maps to Bugsnag
